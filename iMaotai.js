...
```
******************************************/
var $ = new Env('i茅台自动预约'), service = $.http
// 开发方便兼容node
...
// -----------------------------------------------------------------------------------------
//手动配置
// imaotaiParams='{"headers":{"MT-Network-Type":"WIFI","MT-R":"clips_OlU6TmFRag5rCXwbNAQ/Tz1SKlN8THcecBp/HGhHdw==","Cookie":"__bid_n=18bf6c07a16fb1d1304207","Accept-Encoding":"gzip, deflate, br","Connection":"keep-alive","BS-DVID":"WO8vFkOORTXXIhQidi51KKCCVNSqp-cEbUnqcqKwT2ftmzyohC_RmhmVZUBbTRm8Nusl3ppwItoSlgqo-Otq1cQ","MT-Device-ID":"BED21DB1-C282-4C21-B726-27B888CE02A1","MT-Request-ID":"170383711200835322","User-Agent":"iOS;15.2;Apple;iPhone 12 ProMax","MT-User-Tag":"0","MT-Bundle-ID":"com.moutai.mall","Host":"app.moutai519.com.cn","MT-Team-ID":"","MT-APP-Version":"1.5.4","Accept-Language":"zh-Hans-CN;q=1, en-CN;q=0.9","Accept":"*/*","MT-Token":"eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJtdCIsImV4cCI6MTcwNDc4MjY4NywidXNlcklkIjoxMTM3NTkzNjY5LCJkZXZpY2VJZCI6IkJFRDIxREIxLUMyODItNEMyMS1CNzI2LTI3Qjg4OENFMDJBMSIsImlhdCI6MTcwMjE5MDY4N30.c3KDT1mvTn_PAGJuBfw6NAaMluA9jjRiL3XQS3w9Lpo"},"userId":1137593669}'
// var province = '' // 省份
// var city =  '' // 城市
// var location = '' // 地址经纬度
var address = '江苏省南京市鼓楼区金陵新九村'
var shopid = ''
...
!(async () => {
    ...
    if ($request && typeof $request === 'object') {
        ...
        console.log(`获取到抓包数据：` + JSON.stringify($request))
    ...
    $.log(`获取到抓包数据：${JSON.stringify(imaotaiParams)}`)
    if (!province) throw '请在BoxJs中配置省份'
    if (!city) throw '请在BoxJs中配置城市'
    $.log(`获取到：${province} - ${city}`)
    ...
    appointmentTime = await getRandomAppointmentTime() // 获取随机生成的预约时间
    $.log(`当前时间：${await formatTimeWithMilliseconds(current)}`)
    $.log(`预约时间：${await formatTimeWithMilliseconds(appointmentTime)}`)
    // 计算时间差（以毫秒为单位）
    var timeDiff = appointmentTime.getTime() - current.getTime()
    const remainingTime = Math.ceil(timeDiff / 1000);
    $.log(`距离预约时间还有 ${remainingTime} 毫秒`);
    var executed = false
    const threshold = 1000 // 设置一个阈值，单位为毫秒
    const threshold2 = 2000 // 设置一个阈值，单位为毫秒
    do {
        
        var currentTime = new Date() // 获取当前时间
        currentTime.setSeconds(currentTime.getSeconds() + 1); // 设置当前时间加1秒
        if (!executed && new Date() >= appointmentTime) {
            for (var key in itemMap) {
                if (key === '10213') {
                    $.log(`开始预约: ${key}:'${itemMap[key]}'`)
                    itemCode = key
                    var result = await TodoMT(itemCode, appointmentTime)
                    var randomseconds = getRandom(300, 8000) // 生成0到6000之间的随机整数作为毫秒数
                    $.log(`等待${randomseconds}秒`)
                    await delay(randomseconds); // 等待几秒
                }
                if (key === '10214') {
                    $.log(`开始预约: ${key}:'${itemMap[key]}'`)
                    itemCode = key
            ...
            // if (currentTime.getTime() - new Date().getTime() > threshold||currentTime.getTime() - new Date().getTime()< threshold2) {
            //     const remainingTime = Math.ceil((appointmentTime.getTime() - new Date().getTime()) / 1000);
            //     $.log(`距离预约时间还有 ${remainingTime} 秒`);
            // }
        }
    } while (!executed)
 ...
 * 根据详细地址查询经纬度
 */
async function TodoMT() {
    var msg = `预约时间已到：${await formatTimeWithMilliseconds(appointmentTime)}`
    $.log(msg)
    const notify = $.msg($.name, '', msg)

    var { headers, userId } = imaotaiParams
    maotai.headers = Object.assign(maotai.headers, headers)
    maotai.userId = userId
    $.log(`获取到替换之后请求头 ：${JSON.stringify(maotai.headers)}`)
    //获取今日主页
    await maotai.getIndex()
    ...
    $.log(`获取到sessionId值：${maotai.sessionId}`)
    //获取tab页面
    await maotai.getTab()
    //获取到商铺地图数据
    ...
    $.log(`获取到商铺地图数据：${JSON.stringify(dictionary)}`)
    maotai.dictionary = dictionary
    if (!maotai.shopId) {
        //随机个数，判断走最近的店铺还是量最大的店铺
        var randomNumber = Math.random();
        switch (true) {
            case randomNumber < 0.5:
                var getshops = await maotai.getNearbyStore()
                var nearStore = getshops.sort((a, b) => a.distance - b.distance)[0]
                maotai.shopId = nearStore.shopId
                break;
            case randomNumber >= 0.5:
                var shopId = await maotai.getRandomStore()
                maotai.shopId = shopId
                break;
        }
    }
    $.log(`获取到店铺id：${maotai.shopId}`)

    //await maotai.doReserve()
    var randomseconds = getRandom(3000, 10000) // 生成0到6000之间的随机整数作为毫秒数
    $.log(`预约之后等待${randomseconds}秒再获取耐力值`)
    await delay(randomseconds); // 等待几秒
 ...
 * 获取随机时间
 */
async function getRandomAppointmentTime() {
    var appointmentTime = new Date()
    appointmentTime.setHours(22, 42, 0, 0) // 设置预约时间的小时、分钟、秒和毫秒
    var randomMilliseconds = Math.floor(Math.random() * 60000) // 生成0到59999之间的随机整数作为毫秒数
    appointmentTime.setMilliseconds(randomMilliseconds) // 设置随机毫秒数
    // var randomSeconds = Math.floor(Math.random() * 60); // 生成0到59之间的随机整数作为秒数
    // appointmentTime.setSeconds(randomSeconds); // 设置随机秒数
    return appointmentTime
}
/**
 * 获取精准时间
 */
async function formatTimeWithMilliseconds(date) {
    var hours = date.getHours()
    var minutes = date.getMinutes()
    var seconds = date.getSeconds()
    var milliseconds = Math.floor(date.getMilliseconds() / 10) // 获取十毫秒的单位
    // 判断是否需要在小时、分钟、秒前面补零
    ...
    return new (class {
        constructor() {
            this.headers = {
                ...
                'MT-K': Date.now(),
                'MT-R': `clips_OlU6TmFRag5rCXwbNAQ/Tz1SKlN8THcecBp/HGhHdgw==`,
        ...
        async getIndex() {
            ...
            //     YX_SUPPORT_WEBP: '1'
            // }
            // var req= Object.entries(cookies)

            var _ts = new Date().setHours(0, 0, 0, 0)
            var options = {
                url: `https://static.moutai519.com.cn/mt-backend/xhr/front/mall/index/getV2/${_ts}`,
                headers: this.headers
            }
            var { body: response } = await service.get(options)
            var { code, data, message } = JSON.parse(response)
            $.log(`获取主页数据：${data}`)
            if (code !== 2000) throw `获取Index失败, ${message}`
            var { logoUrl } = data
            return logoUrl
        }
        // 获取tab
        async getTab() {
            ...
            var { body: response } = await service.get(
                `https://static.moutai519.com.cn/xhr/front/user/community/tab`
            )
            var { code, data, message } = JSON.parse(response)
            if (code !== 2000) throw `获取tab失败, ${message}`
            var { show } = data
            return show
        }
        // 获取今日sessionId
        ...
        async getRandomStore() {
            ...
            // 获取当地店铺
            var nearShop = getNearbyStore()
            // 预约项投放量最大的店铺列表
            var bestReserveShop = nearShop
                .filter((item) => _shops.find((_item) => _item.shopId === item.shopId))
                .sort((a, b) => b.items.inventory - a.items.inventory)
            if (bestReserveShop.length < 2) {
                return bestReserveShop.shopId
            }
            // 获取前一半店铺的长度
            var halfLength = Math.ceil(bestReserveShop.length / 2)
            // 生成一个随机索引，范围是 0 到前一半店铺的长度
            var randomIndex = Math.floor(Math.random() * halfLength)
            // 获取随机选择的店铺的 shopId
            var randomShopId = bestReserveShop[randomIndex].shopId
            return randomShopId
        }
        // 获取最近店铺
